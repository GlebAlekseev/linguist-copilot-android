package pro.linguistcopilot.features.reader.presentation.view3.provider
...
import pro.linguistcopilot.features.reader.presentation.view3.BookContent
import pro.linguistcopilot.features.reader.presentation.view3.ImageProvider
import pro.linguistcopilot.features.reader.presentation.view3.ReadBook
import pro.linguistcopilot.features.reader.presentation.view3.RealPathUtil
            ...
            if (true) {
                ...
                while (matcher.find()) {
                    ...
                    durY = setTypeImage(
                        ...
                        absStartX, durY, textPages, stringBuilder, null
                    )
                    start = matcher.end()
                }
                if (start < content.length) {
                    ...
                    if (text.isNotBlank()) {
                        setTypeText(
                            book, absStartX, durY,
                            if (true) text + reviewChar else text,
                            textPages,
                            stringBuilder,
                            contentPaint,
                            contentPaintTextHeight,
...
//                }
        ...
        for (lineIndex in 0 until layout.lineCount) {
            ...
            when {
                ...
                lineIndex == layout.lineCount - 1 -> {
                    ...
                    val startX = if (
                        isTitle &&
                        (false || emptyContent || isVolumeTitle)
                    ) {
    ...
    fun upStyle() {
        ...
        //间距
        lineSpacingExtra = 12 / 10f
        paragraphSpacing = 2
        titleTopSpacing = 0.dpToPx()
        titleBottomSpacing = 0.dpToPx()
        val bodyIndent = "　　"
        indentCharWidth = StaticLayout.getDesiredWidth(bodyIndent, contentPaint) / bodyIndent.length
        titlePaintTextHeight = titlePaint.textSize
        contentPaintTextHeight = contentPaint.textSize
    ...
    private fun getPaints(typeface: Typeface?): Pair<TextPaint, TextPaint> {
        ...
        tPaint.color = Color.parseColor("#3E3D3B")
        tPaint.letterSpacing = 0.1f
        tPaint.typeface = titleFont
        tPaint.textSize = (20 + 0).toFloat().spToPx()
        tPaint.isAntiAlias = true
        //正文
        val cPaint = TextPaint()
        cPaint.color = Color.parseColor("#3E3D3B")
        cPaint.letterSpacing = 0.1f
        cPaint.typeface = textFont
        cPaint.textSize = 20.toFloat().spToPx()
